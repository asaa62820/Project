...
namespace wms
{
    public partial class Invoice : BaseForm
    {
        public Invoice()
        {
            InitializeComponent();
...
truncate table [dbo].[t_imp_13];");
            ...
            group_file = null;
            group_file = DB.GetDataGroup("select fromid, site_code from t_base_from where status='1';");
            ...
            #endregion
            string ls_load_file = "select fileid,table_name,file_type,file_dir,(select site_code from t_base_from where fromid=[t_base_file].fromid) as file_dir2,file_name,field_count,include_head,field_Separator,imp_check_file_time from [t_base_file] where file_type in ('txt','csv','xls') and status='1'";
            ...
            if (group_files.Table.Rows.Count > 0)
            {
                for (int i = 0; i < group_files.Table.Rows.Count; i++)
                {
                    ...
                    string ls_file_FullName = "z:\\Invoice Printing\\" + DateTime.Now.ToString("yyyy-MM") + "\\" + DateTime.Now.ToString("yyyy-MM-dd")+"\\"+ group_files.Table.Rows[i]["file_dir2"].ToString() +"\\"+ group_files.Table.Rows[i]["file_name"].ToString();

                    string ls_old_FullName = group_files.Table.Rows[i]["file_dir"].ToString() + "\\" + group_files.Table.Rows[i]["file_name"].ToString();
...
");
            ...
            myConn.Open();
            string sqlstr = @"select x.FROMID,(select site_name from [t_base_from] where fromid=x.fromid) as site_name,x.order_num,x.item_num from (
select FROMID,count(distinct order_id) as order_num ,count(order_id) as item_num  from [t_biz_order] where 
Order_DAY=convert(varchar(8), getdate(), 112)
group by fromid) x";
                ...
                for (int i_order = 0; i_order < group_order.Table.Rows.Count; i_order++)
                {
                    ...
                    if (group_invoice.Table.Rows.Count > 0)
                    {
                        for (int i = 0; i < group_invoice.Table.Rows.Count; i++)
                        {
                            ...
                            string ls_phone = group_invoice.Table.Rows[i]["phone"].ToString();
                            ...
                            string ls_Retailer_Order_Number = group_invoice.Table.Rows[i]["Retailer_Order_Number"].ToString();
                            string ls_Return_Contact_Name = group_invoice.Table.Rows[i]["Return_Contact_Name"].ToString();
                            string ls_Return_Address_1 = group_invoice.Table.Rows[i]["Return_Address_1"].ToString();
                            string ls_Return_Address_2 = group_invoice.Table.Rows[i]["Return_Address_2"].ToString();
                            string ls_Return_City = group_invoice.Table.Rows[i]["Return_City"].ToString();
                            string ls_Return_State_Or_Province = group_invoice.Table.Rows[i]["Return_State_Or_Province"].ToString();
                            string ls_Return_Postal_Code = group_invoice.Table.Rows[i]["Return_Postal_Code"].ToString();
                            string ls_Return_Country_Code = group_invoice.Table.Rows[i]["Return_Country_Code"].ToString();
                            string ls_Return_Phone = group_invoice.Table.Rows[i]["Return_Phone"].ToString();
                            //string ls_Return_Alternate_Phone = group_invoice.Table.Rows[i]["Return_Alternate_Phone"].ToString();
...
800-843-2446. For an international return, email international@overstock.com or call 00-1-919-576-9926 for instructions.</span>
        ...
        private void insert_access_overstock()
        {
            ...
            //string ls_ov_db = "Z:\\Invoicing Frontends\\Over Stock Invoicing_FrontEnd_v1.accdb";
            DataGroup group_invoice = new DataGroup();
            group_invoice = null;
...
order by item,[shipping_name]";
            ...
            group_invoice = DB.GetDataGroup(ls_get_ov_order);
            if (group_invoice.Table.Rows.Count > 0)
            {
                for (int i = 0; i < group_invoice.Table.Rows.Count; i++)
                {
                    ...
                    string ls_Retailer_Order_Number = group_invoice.Table.Rows[i]["Retailer_Order_Number"].ToString();
                    string ls_SOFS_Order_Line_Number = group_invoice.Table.Rows[i]["SOFS_Order_Line_Number"].ToString();
                    string ls_oid = group_invoice.Table.Rows[i]["Retailer_Order_Number"].ToString();
                    string ls_pd1 = group_invoice.Table.Rows[i]["pd1"].ToString();
                    string ls_pd2 = group_invoice.Table.Rows[i]["pd2"].ToString();
                    string ls_qty = group_invoice.Table.Rows[i]["qty"].ToString();
                    string ls_sku = group_invoice.Table.Rows[i]["sku"].ToString();
                    string ls_item = group_invoice.Table.Rows[i]["item"].ToString();
                    string ls_address = group_invoice.Table.Rows[i]["address"].ToString();
                    string ls_cost = group_invoice.Table.Rows[i]["cost"].ToString();
                    string ls_item_shipping_price = group_invoice.Table.Rows[i]["item_shipping_price"].ToString();
                    string ls_item_price = group_invoice.Table.Rows[i]["item_price"].ToString();
                    string ls_shipping_name = group_invoice.Table.Rows[i]["shipping_name"].ToString();
                    string ls_status = group_invoice.Table.Rows[i]["status"].ToString();
                    string ls_SOFS_ID = group_invoice.Table.Rows[i]["order_id"].ToString();
